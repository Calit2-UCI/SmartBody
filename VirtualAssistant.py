...
----------------------------
	* line = input_file.readline().rstrip()	# need to parse file
	* if line:
		gaze(direction)
		speak(line)
...
Log File Format:
----------------
	* Iteration:(zero = default);Expression:(0,1,2);TTS:(string from Python script)

FUTURE:
-------
	* Change the level of positive and negative feedback (implement with extra parmeters)
	* Separate modules
'''
...
'''
RUN sbgui.exe
SELECT Window > Speech Relay
	Keep it running - allows character to speak
SELECT File > Run Script...
SELECT VirtualAssistant.py

Script will run and constantly check for new lines in Music Glove
CSV output file. Acts appropriately based on if there is a new line of input.
'''
